...
def wordfreq_viz(
    ...
    fig.vbar(
        x="index",
        top=col,
        fill_color=cfg.wordfreq.color,
        line_color=cfg.wordfreq.color,
        width=0.9,
        source=df,
    )
...
def bar_viz(
    ...
    fig = Figure(
        ...
        tools="hover",
        x_range=list(df.index),
        y_axis_type=cfg.bar.yscale,
    )
    fig.vbar(
        x="index",
        width=0.9,
        top=col,
        fill_color=cfg.bar.color,
        line_color=cfg.bar.color,
        bottom=0.01,
        source=df,
    )
    ...
    if show_yticks and cfg.bar.yscale == "linear":
        _format_axis(fig, 0, df[col].max(), "y")
    return fig
...
def pie_viz(
    ...
    if cfg.pie.colors is None:
        color_list = list((CATEGORY20 * (len(df) // len(CATEGORY20) + 1))[0 : len(df)])
    else:
        color_list = list(cfg.pie.colors[0 : len(df)])
...
def kde_viz(
    ...
    hist = fig.quad(
        ...
        bottom=bottom,
        top="dens",
        fill_color=cfg.kde.hist_color,
        line_color=cfg.kde.hist_color,
    )
    ...
    pdf = kde(pts_rng)
    line = fig.line(x=pts_rng, y=pdf, line_color=cfg.kde.line_color, line_width=2, alpha=0.5)
...
def qqnorm_viz(
    ...
    fig.circle(
        ...
        y=qntls,
        size=3,
        color=cfg.qqnorm.point_color,
    )
    vals = np.concatenate((theory_qntls, qntls))
    fig.line(x=[vals.min(), vals.max()], y=[vals.min(), vals.max()], color=cfg.qqnorm.line_color)
    tweak_figure(fig, "qq")
...
def render_cat(itmdt: Intermediate, cfg: Config) -> Dict[str, Any]:
    ...
    if cfg.pie.enable:
        fig, color_list = pie_viz(
            data["pie"].to_frame(), data["nrows"], col, plot_width, plot_height, cfg
        )
        tabs.append(fig)
...
def render_num(itmdt: Intermediate, cfg: Config) -> Dict[str, Any]:
    ...
    if cfg.qqnorm.enable:
        if data["qntls"].any():
            ...
            tabs.append(qqnorm_viz(qntls, mean, std, col, plot_width, plot_height, cfg))
    ...
    if cfg.box.enable:
        ...
        tabs.append(box_viz(df, col, plot_width, plot_height, cfg))
...
def render_cat_num(itmdt: Intermediate, cfg: Config) -> Dict[str, Any]:
    ...
    if cfg.box.enable:
        ...
        tabs.append(box_viz(df, x, plot_width, plot_height, cfg, y, data["ttl_grps"]))
...
def render_two_num(itmdt: Intermediate, cfg: Config) -> Dict[str, Any]:
    ...
    if cfg.box.enable:
        ...
        df.columns = df.columns.get_level_values(1)
        df.columns = ["grp"] + list(df.columns[1:])
        tabs.append(box_viz(df, x, plot_width, plot_height, cfg, y))
